...
which makes a random number or chooses a random historical event and offers the player to guess or find it, respectively
 */
...
public class numbers {
    public static void main(String[] args) {
        ...
        String[][] historicalEvents = null; //the 2D array to store the names and corresponding years of the events
        //initialized beforehand not to get "Variable might not be initialized" error thrown

        int inputNum = -1; //input integer by the user
        boolean inputIsCorrect = false; //to check if input is an integer
        int[] arrInputNums = new int[0]; //to collect the inputs
        ...
        if(game==1) {
            randomNum = rnd.nextInt(101); //since 101 is exclusive
            correctAnswer = randomNum;
        }
        else { //not taking into account an input for game, different than 1 or zero
        ...
        while(true) {
            ...
            //if the game is 2, print the corresponding name (question) of the random event
            if(game==2) {
            ...
            do {
                ...
                try {
                    ...
                    inputNum = in.nextInt();
                    inputIsCorrect = true; //will terminate do while loop
                }
            ...
            //save the input number to the last position of the array, which had zero value
